function [dw, cx, cy, x_zero, y_zero] = calibrate_SPIM()
    ...
    dw       = 30;    % distance between proj and chamber in mm
    cx       = 20; % px/mm x
    cy       = 20;    % px/mm y
    x_zero   = 423;   % zero x axis in px
    y_zero   = 250;   % zero y axis in px
    ...
    while(keepgoing)
        ...
        xy = [1 projWidth x_zero x_zero;
            y_zero y_zero 1 projHeight];
    ...
    while(keepgoing)
        ...
        fp = flatProjection(dw,cx,cy,x_zero,y_zero);
        theta = fp.get_theta(Xq);
        phi = fp.get_phi(Yq);
        
        im_grid = zeros(projHeight,projWidth);
        ind = find(abs(mod(tan(theta)+gx,10/dw)) < 0.01 | abs(mod(tan(phi)+gy,10/dw)) < 0.01);
        im_grid(ind) = 255;
